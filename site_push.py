...
class SSLCertificateTask(BaseTask):
    def __init__(self):
        ...
        self.title = "SSL 证书过期提醒"
        self.template_name = "SSL 证书过期提醒"
    ...
    def get_push_data(self, task_id: str, task_data: dict) -> Optional[dict]:
        ...
        if domain_list:
            msg_list.append(f"⚠️ 有 {len(domain_list)} 个域名 SSL 证书即将过期\n")
        if mail_list:
...
class SSHLoginErrorTask(BaseTask):
    ...
    def __init__(self):
        ...
        self.template_name = "SSH 登录失败告警"
        self.title = "SSH 登录失败告警"
